...
import frc.robot.commands.cargoarm.*;
...
public class CargoArm extends Subsystem {
    ...
    public CargoArm() {
        ...
        // PID
        cargoArmPID.setP(RobotMap.CARGO_ARM_PIDF[0], RobotMap.PID_SLOT);
        cargoArmPID.setI(RobotMap.CARGO_ARM_PIDF[1], RobotMap.PID_SLOT);
        cargoArmPID.setD(RobotMap.CARGO_ARM_PIDF[2], RobotMap.PID_SLOT);
        cargoArmPID.setFF(RobotMap.CARGO_ARM_PIDF[3], RobotMap.PID_SLOT);
        cargoArmPID.setIZone(0.0);

        // Smart Motion
        cargoArmPID.setP(RobotMap.CARGO_ARM_PIDF_SM[0], RobotMap.SM_SLOT);
        cargoArmPID.setI(RobotMap.CARGO_ARM_PIDF_SM[1], RobotMap.SM_SLOT);
        cargoArmPID.setD(RobotMap.CARGO_ARM_PIDF_SM[2], RobotMap.SM_SLOT);
        cargoArmPID.setFF(RobotMap.CARGO_ARM_PIDF_SM[3], RobotMap.SM_SLOT);
        cargoArmPID.setSmartMotionMaxVelocity(RobotMap.CARGO_ARM_MAX_VEL, RobotMap.SM_SLOT);
        cargoArmPID.setSmartMotionMinOutputVelocity(RobotMap.CARGO_ARM_MIN_VEL, RobotMap.SM_SLOT);
        cargoArmPID.setSmartMotionMaxAccel(RobotMap.CARGO_ARM_MAX_ACC, RobotMap.SM_SLOT);
        cargoArmPID.setSmartMotionAllowedClosedLoopError(RobotMap.CARGO_ARM_PID_TOLERANCE, RobotMap.SM_SLOT);
        ...
        switch(state) {
            ...
            case PID:
                if(currentPIDSetpoint == -1257) {
                    ...
                    cargoArmPID.setReference(currentPIDSetpoint, ControlType.kPosition, RobotMap.PID_SLOT,
            ...
            case SMART_MOTION:
                if(currentPIDSetpoint == -1257) {
                    ...
                    cargoArmPID.setReference(currentPIDSetpoint, ControlType.kSmartMotion, RobotMap.SM_SLOT,
                            RobotMap.CARGO_ARM_ARB_F * Math.cos(getArmAngle()));
